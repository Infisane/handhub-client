  ...
  return (
    ...
    >
      <div className="mb-3 flex items-start gap-2">
        <FileText size={16} style={{ color: 'var(--lagoon-deep)' }} aria-hidden="true" />
        <div className="min-w-0 flex-1">
          <p className="text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--sea-ink-soft)' }}>
            Quote Proposal
          </p>
          <p className="mt-0.5 text-sm font-semibold" style={{ color: 'var(--sea-ink)' }}>
            {message.title}
          </p>
        ...
        <span className="shrink-0 text-lg font-bold" style={{ color: 'var(--sea-ink)' }}>
          {message.currency}
      ...
      {isPending ? (
        ...
        <div
          ...
          style={{
            background: isAccepted ? 'rgba(79,184,178,0.12)' : 'rgba(239,68,68,0.08)',
...
function ImageBubble({ message, onZoom }: { message: ImageMessage; onZoom: (msg: ImageMessage) => void }) {
  return (
    <div className="group relative max-w-[200px] cursor-pointer overflow-hidden rounded-xl" onClick={() => onZoom(message)}>
      <img
        src={message.src}
        alt={message.alt}
...
export function ChatActionCenter({
  ...
  const [internalMessages, setInternalMessages] = useState<Message[]>(() => DEMO_MESSAGES)
  ...
  function handleQuoteAccept(quoteId: string) {
    ...
    if (!externalMessages) {
      setInternalMessages(prev =>
        prev.map(m => (m.id === quoteId && m.type === 'quote' ? { ...m, status: 'accepted' } : m))
      )
    }
  }
  ...
  function handleQuoteDecline(quoteId: string) {
    ...
    if (!externalMessages) {
      setInternalMessages(prev =>
        prev.map(m => (m.id === quoteId && m.type === 'quote' ? { ...m, status: 'declined' } : m))
      )
    }
  }
    ...
    >
      ...
      >
        ...
        <div className="ml-auto flex items-center gap-1">
          <MapPin size={14} style={{ color: 'var(--lagoon-deep)' }} aria-hidden="true" />
      ...
      {/* Message list */}
      <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-4 py-4" role="log" aria-live="polite" aria-label="Chat messages">
        {messages.map(msg => {
          if (msg.type === 'ai') {
            return (
              <div
                ...
                style={{ background: 'var(--ai-gradient)', border: '1px solid var(--ai-border)' }}
                aria-label="Handhub AI Assistant"
                role="note"
              >
                <p className="mb-1 text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--ai-primary)' }}>
                  Handhub AI
                </p>
                <p className="text-sm" style={{ color: 'var(--sea-ink)' }}>
                  {msg.content}
                </p>
                <p className="mt-1.5 text-right text-[10px]" style={{ color: 'var(--sea-ink-soft)' }}>
            ...
            return (
              <div key={msg.id} className="flex gap-2">
                ...
                <div className="max-w-[80%] flex-1">
                  ...
                  />
                  <p className="mt-1 text-[10px]" style={{ color: 'var(--sea-ink-soft)' }}>
                    <Clock size={10} className="mr-0.5 inline" aria-hidden="true" />
                    {formatTime(msg.timestamp)}
                  </p>
                </div>
              </div>
          ...
          return (
            <div
              key={msg.id}
              className={cn('flex items-end gap-2', isCustomer ? 'flex-row-reverse' : 'flex-row')}
            >
              {!isCustomer && <Avatar initial={artisanInitial} size={28} />}

              <div className={cn('flex max-w-[72%] flex-col gap-1', isCustomer && 'items-end')}>
                {msg.type === 'image' ? (
                  ...
                  <div
                    ...
                    style={{
                      background: isCustomer ? 'var(--lagoon-deep)' : 'var(--surface-strong)',
                ...
                <p className="text-[10px]" style={{ color: 'var(--sea-ink-soft)' }}>
                  <Clock size={10} className="mr-0.5 inline" aria-hidden="true" />
                  <time dateTime={msg.timestamp.toISOString()}>{formatTime(msg.timestamp)}</time>
                </p>
              </div>
            </div>
          )
      ...
      >
        ...
        >
          ...
          >
            <Paperclip size={18} style={{ color: 'var(--sea-ink-soft)' }} aria-hidden="true" />
          </button>
          <input
            ref={fileInputRef}
            type="file"
        ...
        <p className="mt-1.5 text-center text-[10px]" style={{ color: 'var(--sea-ink-soft)' }}>
      ...
      <dialog
        ...
        aria-label={zoomedImage ? `Zoomed image: ${zoomedImage.alt}` : 'Image viewer'}
      >
        {zoomedImage && (
          <div className="relative">
            ...
            <p className="mt-2 text-center text-sm text-white/80">{zoomedImage.alt}</p>
          </div>
        )}
      </dialog>
    </div>
...
const DEMO_MESSAGES: Message[] = [
  {
    id: '1',
    type: 'ai',
    content: 'I found 3 verified electricians near you. Chukwuemeka Obi is available immediately and has a 4.9★ rating from 142 jobs. Connecting you now.',
    timestamp: t(18),
  },
  {
    id: '2',
    type: 'text',
    sender: 'artisan',
    content: 'Hello! I just saw your request about the smoking panel. That sounds like it could be a tripped breaker or a loose live wire — both need urgent attention. Can you describe what you hear or smell?',
    timestamp: t(15),
  },
  {
    id: '3',
    type: 'text',
    sender: 'customer',
    content: "There's a burning smell and one socket near the panel is sparking occasionally.",
    timestamp: t(13),
  },
  {
    id: '4',
    type: 'text',
    sender: 'artisan',
    content: "Please switch off the main breaker immediately if you can do so safely. I can head over by 4:00 PM. Is that fine?",
    timestamp: t(12),
  },
  {
    id: '5',
  ...
  },
  {
    ...
    description: 'Full panel inspection, fault diagnosis, and socket repair. Parts billed separately if needed.',
    amount: 45.0,
    currency: '$',
    status: 'pending',
    timestamp: t(8),
