...
export function AISearchInput({ onSearch, className }: AISearchInputProps) {
	...
	useEffect(() => {
		...
		const id = setInterval(
			() => setPromptIndex((i) => (i + 1) % COMPUTING_PROMPTS.length),
			2200,
		);
	...
	const borderStyle =
		isFocused || isComputing ? "var(--ai-border)" : "var(--line)";
	const bgStyle =
				...
				>
					...
					<input
						...
						className={cn(
							...
							isComputing &&
								"cursor-wait placeholder:animate-[prompt-blink_2.2s_ease-in-out_infinite]",
						)}
