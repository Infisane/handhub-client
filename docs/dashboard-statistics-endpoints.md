...
> **Since this doc was first written:** `POST /api/wallet/withdraw` is no longer a one-shot
> balance mutation — it's now a real, admin-reviewed bank payout. See §3's withdrawal note below
> and **`docs/wallet-withdrawal-implementation.md`** for the full flow before wiring up the
> Payments page's withdraw action.
...
Withdrawal ledger rows now come in pairs when a payout fails or is rejected — a `debit` /
`withdrawal` row at request time, and a matching `credit` / `refund` row if it doesn't go
through:
...
> **`POST /api/wallet/withdraw` is not a one-shot balance mutation anymore.** It now requires a
> verified bank account on file (`POST /api/wallet/bank-account`), debits the wallet immediately
> but only _requests_ a payout (`status: "pending"`), and needs an admin to approve it before any
> money actually moves — the response is a withdrawal object with a `status`, not an updated
> wallet. Wiring up the Payments page's withdraw action means building the bank-account step and
> a pending/status state, not just calling the old endpoint. See
> **`docs/wallet-withdrawal-implementation.md`** for the full request/response contract and every
> status transition, live-captured.
...
[
    {
        ...
        "provider": {
            "id": "98e70c78-...",
            "businessName": "Smoke Test Repairs",
            "averageRating": "5.0",
            "...": "..."
        },
        "serviceTitle": "Kitchen sink plumbing",
